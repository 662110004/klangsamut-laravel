    ...
    <div class="user-layout-container">
        ...
        <header class="user-header">
            ...
            <div class="user-header-left">
                <a href="{{ route('books.index') }}" class="sidebar-logo" style="height: 100%; border: none; padding: 0;">
                ...
                <div class="user-header-nav">
                    <a href="{{ route('books.index') }}" class="user-nav-link {{ request()->routeIs('books.*') ? 'active' : '' }}">Browse All Books</a>
                </div>
            </div>
        ...
        <main class="user-main-content">
            ...
            <div class="user-grid-container">
                <aside class="user-genres-sidebar">
                    <h3 class="sidebar-genre-title">All Genres</h3>
                    <ul class="genre-list">
                        @foreach($categories as $category)
                        <li>
                            <a href="{{ route('categories.show', $category->code) }}"
                                class="genre-link {{ request()->is('categories/' . $category->code) ? 'active' : '' }}">
                                {{ $category->name }}
                            </a>
                        </li>
                        @endforeach
                ...
                <div class="user-grid-content">
                    {{ $slot }}
                </div>
            </div>
            @endif @include('layouts.partials.footer-user')
